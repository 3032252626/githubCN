// ==UserScript==
// @name         GitHub CN 汉化
// @namespace    https://github.com/3032252626/githubCN
// @version      1.6.3
// @description  GitHub 汉化插件 - 将 GitHub 界面翻译为中文
// @author       3032252626
        ...
        while (currentNode) {
          if (currentNode.nodeType === 3) {
            ...
            if (dataMap.has(key1)) {
              ...
              let m;
              if ((m = key1.match(/^(\d+) minutes? ago$/)))
                currentNode.textContent = m[1] + "分钟前";
              else if ((m = key1.match(/^(\d+) hours? ago$/)))
                currentNode.textContent = m[1] + "小时前";
              else if ((m = key1.match(/^(\d+) days? ago$/)))
                currentNode.textContent = m[1] + "天前";
              else if ((m = key1.match(/^(\d+) months? ago$/)))
                currentNode.textContent = m[1] + "个月前";
              else if ((m = key1.match(/^(\d+) years? ago$/)))
                currentNode.textContent = m[1] + "年前";
              else if ((m = key1.match(/^(\d+) Commits$/)))
                currentNode.textContent = m[1] + " 次提交";
              else if ((m = key1.match(/^(\d+) Branches$/)))
                currentNode.textContent = m[1] + " 个分支";
            }
          } else {
            ...
            if (tag === "RELATIVE-TIME" || tag === "TIME-AGO") {
              ...
              if (text) {
                if (dataMap.has(text)) {
                  currentNode.textContent = dataMap.get(text);
                } else {
                  let m;
                  if ((m = text.match(/^(\d+) minutes? ago$/)))
                    currentNode.textContent = m[1] + "分钟前";
                  else if ((m = text.match(/^(\d+) hours? ago$/)))
                    currentNode.textContent = m[1] + "小时前";
                  else if ((m = text.match(/^(\d+) days? ago$/)))
                    currentNode.textContent = m[1] + "天前";
                  else if ((m = text.match(/^(\d+) months? ago$/)))
                    currentNode.textContent = m[1] + "个月前";
                  else if ((m = text.match(/^(\d+) years? ago$/)))
                    currentNode.textContent = m[1] + "年前";
                }
              }
            }
